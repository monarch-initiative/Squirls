...
package org.monarchinitiative.threes.core.scoring.conservation.bbfile;

import org.monarchinitiative.threes.core.scoring.conservation.util.LittleEndianInputStream;
...
public class BigWigSectionHeader {
    ...
    private final int chromID;       // Chromosome/contig Numerical ID from BBFile Chromosome B+ tree

    private final int chromStart;    // starting base position

    private final int chromEnd;      // ending base position

    private final int itemStep;      // number of base spaces between fixed items

    private final int itemSpan;      // number of bases in fixed step items

    private WigItemType itemType; // type of data items: 1 = bedGraph, 2 = varStep, 3 = fixedStep

    private final byte reserved;     // reserved; currently = 0

    private final short itemCount;   // number of data items in this chromosome section

    private final boolean isValidType;    // indicates a if a valid Wig item type was read
