...
import org.monarchinitiative.threes.core.scoring.conservation.util.SeekableStream;
...
public class BigWigDataBlock {
    ...
    private final long fileOffset;       // Wig data block file offset

    private final long leafDataSize;     // byte size for data block specified in the R+ leaf

    private final boolean isLowToHigh;   // if true, data is low to high byte order; else high to low

    // defines the bigWig data source
    private final HashMap<Integer, String> chromosomeMap;  // map of chromosome ID's and corresponding names

    private final RPTreeLeafNodeItem leafHitItem;   // R+ leaf item containing data block location

    // uncompressed byte stream buffer and readers
    private final byte[] wigBuffer;      // buffer containing leaf block data uncompressed
