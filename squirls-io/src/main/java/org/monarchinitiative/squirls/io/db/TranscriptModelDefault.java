...
/**
 * A {@link TranscriptModel} implementation local to the `io` module.
 */
class TranscriptModelDefault extends BaseGenomicRegion<TranscriptModelDefault> implements TranscriptModel {
    ...
    private TranscriptModelDefault(Contig contig,
        ...
        this.exons = Objects.requireNonNull(exons, "Exons cannot be null");
        if (exons.isEmpty()) {
    ...
    static class Builder {
        ...
        Builder contig(Contig contig) {
            this.contig = contig;
            return this;
        }

        Builder strand(Strand strand) {
            this.strand = strand;
            return this;
        }
        ...
        Builder accessionId(String accessionId) {
            if (this.accessionId == null) this.accessionId = accessionId;
            return this;
        }

        Builder hgvsSymbol(String hgvsSymbol) {
            if (this.hgvsSymbol == null) this.hgvsSymbol = hgvsSymbol;
            return this;
        }

        Builder setExon(int n, int start, int end) {
            exons.put(n, GenomicRegion.zeroBased(contig, start, end));
            return this;
        }

        Builder cdsStart(int cdsStart) {
            this.cdsStart = cdsStart;
            return this;
        }

        Builder cdsEnd(int cdsEnd) {
            this.cdsEnd = cdsEnd;
            return this;
        }

        TranscriptModelDefault build() {
            GenomicRegion cdsRegion = (cdsStart >= 0 && cdsEnd >= 0) ? GenomicRegion.zeroBased(contig, cdsStart, cdsEnd) : null;

            List<GenomicRegion> sorted = exons.keySet().stream().sorted().map(exons::get).collect(Collectors.toUnmodifiableList());

            return new TranscriptModelDefault(contig, strand, CoordinateSystem.ZERO_BASED, Position.of(start), Position.of(end),
                    accessionId, hgvsSymbol, cdsRegion != null, cdsRegion, sorted);
        }
