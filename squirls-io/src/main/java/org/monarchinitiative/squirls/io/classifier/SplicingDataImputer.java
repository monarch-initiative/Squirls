...
public class SplicingDataImputer<T extends SquirlsFeatures> {
    ...
    public SquirlsFeatures transform(T features) throws PredictionException {
        if (!features.getFeatureNames().containsAll(usedFeatureNames())) {
            ...
            String msg = String.format("Missing at least one required feature. Required: `%s`, Provided: `%s`",
                    ...
                    features.getFeatureNames().stream().collect(Collectors.joining(",", "[", "]")));
            LOGGER.warn(msg);
