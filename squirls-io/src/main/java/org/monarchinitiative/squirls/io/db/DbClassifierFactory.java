...
public class DbClassifierFactory implements ClassifierFactory {
    ...
    public Set<SquirlsClassifierVersion> getAvailableClassifiers() {
        String sql = "select version from SQUIRLS.CLASSIFIER";
    ...
    public int storeClassifier(SquirlsClassifierVersion version, byte[] clfBytes) {
        int updated = 0;
        String sql = "insert into SQUIRLS.CLASSIFIER(version, data) values (?, ?)";
        try (final Connection connection = dataSource.getConnection();
             final PreparedStatement ps = connection.prepareStatement(sql)) {
    ...
    byte[] readClassifierBytes(SquirlsClassifierVersion version) throws SQLException {
        String sql = "select data from SQUIRLS.CLASSIFIER where version = ?";
        try (final Connection connection = dataSource.getConnection();
             final PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, version.toString());
            try (ResultSet rs = ps.executeQuery()) {
