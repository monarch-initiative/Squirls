...
public class FastaStrandedSequenceServiceTest {
    ...
    @Test
    public void sequenceForRegion_queryCoordinatesPastContigEnd() {
        // one base past contig end
        GenomicRegion query = GenomicRegion.of(GENOMIC_ASSEMBLY.contigByName("1"), Strand.POSITIVE, CoordinateSystem.ONE_BASED, Position.of(1), Position.of(10002));
        StrandedSequence sequence = sequenceService.sequenceForRegion(query);
    ...
    public void sequenceForRegion_unknownContig() {
        // unknown contig
        GenomicRegion query = GenomicRegion.zeroBased(Contig.unknown(), 0, 5);
        StrandedSequence sequence = sequenceService.sequenceForRegion(query);
        assertThat(sequence, is(nullValue()));
    }
