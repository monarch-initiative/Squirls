...
public class IntronLength implements FeatureCalculator {
    ...
    public IntronLength(TranscriptModelLocator locator) {
    ...
    public double score(GenomicVariant variant, TranscriptModel transcript, StrandedSequence sequence) {
        ...
        switch (position) {
            case INTRON:
                ...
                return transcript.introns().get(locationData.getIntronIdx()).length();
            case DONOR:
            case ACCEPTOR:
            case EXON:
                final int exonIdx = locationData.getExonIdx();
                if (transcript.introns().size() - exonIdx > 0) {
                    // the variant is not located in the last exon, let's return length of the downstream intron
                    return transcript.introns().get(exonIdx).length();
                }
            case OUTSIDE:
